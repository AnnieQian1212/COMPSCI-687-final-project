...
def algorithm(alpha, alpha_w, gamma, policy_neurons_per_layer, value_neurons_per_layer):
    ...
    with open(f'./Result/runs_results_a{alpha}_aw{alpha_w}_g{gamma}_p{policy_neurons_per_layer}_v{value_neurons_per_layer}.txt', 'w') as f:
        f.write('')
    ...
    for ep in range(episodes):
        ...
        with open(f'./Result/runs_results_a{alpha}_aw{alpha_w}_g{gamma}_p{policy_neurons_per_layer}_v{value_neurons_per_layer}.txt', 'a') as f:
    ...
    plt.savefig(f'./Result/CartPole_a{alpha}_aw{alpha_w}_g{gamma}_p{policy_neurons_per_layer}_v{value_neurons_per_layer}.png', dpi=150)
    plt.show()
