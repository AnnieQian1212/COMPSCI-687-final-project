...
# alpha = 0.00005    
# gamma = 0.99 
# policy_neurons_per_layer = (32, 32)  # Was (64,64)
# value_neurons_per_layer = (32, 32)   # Was (64,32)
...
def algorithm(alpha, alpha_w, gamma, policy_neurons_per_layer, value_neurons_per_layer):
    ...
    policy = BlackPolicyNet(neurons_per_layer=policy_neurons_per_layer)
    value = BlackValueNet(neurons_per_layer=value_neurons_per_layer)

    results = np.zeros(episodes)

    os.makedirs('./Result', exist_ok=True)
    with open(f'./Result/Blackjack_runs_results_a{alpha}_aw{alpha_w}_g{gamma}_p{policy_neurons_per_layer}_v{value_neurons_per_layer}.txt', 'w') as f:
        f.write('')
    ...
    for ep in range(episodes):
        ...
        with open(f'./Result/Blackjack_runs_results_a{alpha}_aw{alpha_w}_g{gamma}_p{policy_neurons_per_layer}_v{value_neurons_per_layer}.txt', 'a') as f:
    ...
    plt.title('REINFORCE with Baseline on Blackjack-v1')
    ...
    plt.savefig(f'./Result/Blackjack_a{alpha}_aw{alpha_w}_g{gamma}_p{policy_neurons_per_layer}_v{value_neurons_per_layer}.png', dpi=150)
    # plt.show()

    print(f"\nFinal 100-episode average: {np.mean(results[-100:]):.2f}")

# algorithm(0.01,0.01,1,(32,32),(32,32))