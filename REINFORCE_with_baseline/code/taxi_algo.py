...
def algorithm(alpha, alpha_w, gamma, policy_neurons_per_layer, value_neurons_per_layer):
    ...
    policy = TaxiPolicyNet(neurons_per_layer=policy_neurons_per_layer)
    value = TaxiValueNet(neurons_per_layer=value_neurons_per_layer)

    results = np.zeros(episodes)

    os.makedirs('./Result', exist_ok=True)
    with open(f'./Result/Taxi_runs_results_a{alpha}_aw{alpha_w}_g{gamma}_p{policy_neurons_per_layer}_v{value_neurons_per_layer}.txt', 'w') as f:
        f.write('')
    ...
    for ep in range(episodes):
        ...
        with open(f'./Result/Taxi_runs_results_a{alpha}_aw{alpha_w}_g{gamma}_p{policy_neurons_per_layer}_v{value_neurons_per_layer}.txt', 'a') as f:
    ...
    plt.title('REINFORCE with Baseline on Taxi-v3')
    ...
    plt.savefig(f'./Result/Taxi_a{alpha}_aw{alpha_w}_g{gamma}_p{policy_neurons_per_layer}_v{value_neurons_per_layer}.png', dpi=150)
    # plt.show()

    print(f"\nFinal 100-episode average: {np.mean(results[-100:]):.2f}")

# algorithm(0.001,0.001,0.99,(32,16),(32,32))